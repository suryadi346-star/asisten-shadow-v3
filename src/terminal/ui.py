...
class UI:
    ...
    def prompt(message: str, password: bool = False, default: str = "") -> str:
        ...
        Args:
            ...
            password: If True, mask input with dots
            default: Default value
        """
        if password:
            # Use improved password input with visual feedback
            try:
                # Import custom password module
                import os
                import sys
                parent_dir = Path(__file__).parent.parent
                utils_dir = parent_dir / "utils"
                
                # Add utils to path if exists
                if utils_dir.exists():
                    sys.path.insert(0, str(parent_dir))
                    from utils.password_input import get_password_with_dots
                    
                    # Get password with dots
                    pwd = get_password_with_dots(f"\033[36m{message}\033[0m ")
                    return pwd
                else:
                    # Fallback to getpass
                    import getpass
                    console.print(f"[{config.COLOR_PRIMARY}]{message}[/]", end=" ")
